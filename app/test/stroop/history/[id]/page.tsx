...
export default function StroopAttemptDetail() {
  const params = useParams<{ id: string }>();
  const id = params?.id;

  const attempt = useMemo<StroopAttempt | null>(() => {
    const h = loadStroopHistory();
    return h.find((x) => x.id === id) ?? null;
  }, [id]);
  ...
  if (!attempt) {
    return (
      <main className={styles.page}>
        ...
        <Link className={sheet.back} href="/test/stroop/history">← Буцах</Link>
      </main>
    );
  }
  ...
  return (
    <main className={styles.page}>
      <div className={sheet.head}>
        <div>
          <h1 className={styles.h1} style={{ marginBottom: 6 }}>Stroop Attempt дэлгэрэнгүй</h1>
        ...
        <div className={sheet.actions}>
          <button
            ...
            onClick={() => downloadCsv(fileName, buildCsv(attempt))}
          >
            CSV татах
          </button>
          <Link className={sheet.back} href="/test/stroop/history">← Түүх рүү буцах</Link>
        </div>
      </div>
